...
import { Api_Product_tab_basic, Api_Product_tab_height, Api_Product_tab_pro } from 'axios/Axios';
...
function Product_tabs() {
    ...
    const cartArr = []
    const AddToCart = (couses) => {
        const checkCart = cartArr.find((value) => value.name === couses.name)
    ...
    const [apiBasic, setApiBasic] = useState([Product_tabs_lists_bassic])
    useEffect(() => {
        Api_Product_tab_basic
            .getAll()
            .then(res => setApiBasic(res.data))
    }, [Product_tabs_lists_bassic])

    const [apiHeight, setApiHeight] = useState([])
    useEffect(() => {
        Api_Product_tab_height
            .getAll()
            .then(res => setApiHeight(res.data))
    }, [Product_tabs_lists_height])

    const [apiPro, setApiPro] = useState([])
    useEffect(() => {
        Api_Product_tab_pro
            .getAll()
            .then(res => setApiPro(res.data))
    }, [Product_tabs_lists_pro])
                ...
                <Box sx={{ width: '100%' }}>
                    <div className={styles.Product_tabs_header}>
                        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                            <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
                                <Tab label="converst basic" {...a11yProps(0)} />
                                <Tab label="converst truck " {...a11yProps(1)} />
                                <Tab label="converst auth"{...a11yProps(2)} />
                            </Tabs>
                        </Box>
                    </div>
                    <TabPanel value={value} index={0}>
                        <div aria-roledescription="slide" className={styles.Product_tabs_lists} >
                            {
                                apiBasic.map((product) => (
                                    <div key={product.id} className={styles.Product_tabs_item} >
